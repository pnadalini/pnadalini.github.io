...
const ExperienceCard = ({
  ...
  <div className="bg-oxford-blue-2 rounded-xl shadow-lg p-8 hover:shadow-xl transition-shadow duration-300 border border-primary-600">
    <div className="flex flex-col md:flex-row md:items-start md:justify-between mb-6">
      <div>
        <h3 className="text-2xl font-bold text-primary-50 mb-1">{company}</h3>
        <p className="text-primary-200 font-semibold mb-2">{title}</p>
      ...
      <ul className="space-y-3">
        {achievements.map((achievement, index) => (
          <li key={index} className="flex items-start gap-3">
            <FontAwesomeIcon icon={faStar} className="w-5 h-5 text-primary-200 mt-0.5 flex-shrink-0" />
            <span className="text-primary-100 leading-relaxed">{achievement}</span>
          </li>
        ))}
      </ul>
